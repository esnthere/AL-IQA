...
def main(wt):
    ...
    ind,inds_train_new,inds_test_new=second_sel(test_bf,wt,num)
    ind_train=np.concatenate((ind_bf[inds_train_bf],test_bf[ind[inds_train_new]]),axis=0)
    ind_test=test_bf[ind[inds_test_new]]

    rt = 0.1
    num = int(X.shape[0] * rt)
    ind_third,inds_train_third,inds_test_third=third_sel(ind_test,wt,num)
    ...
    for i in range(3):
        ...
        for epoch in range(epochs):
            ...
            if epoch ==20:
                lr = 0.01
            if max_plsp < pl+pl2:
                save_nm = 'livew244_rt25_thirdstep'+str(i)+str(wt)+'.pt'
                max_plsp = pl+pl2
