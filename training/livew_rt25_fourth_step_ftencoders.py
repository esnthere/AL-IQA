...
def first_sel():
    ...
    return ind,inds_train,inds_test

def second_sel(indtest,wt,num):
    ...
    all_data = sio.loadmat('prompt3vote_rt25_secondstep_livew.mat')
    op0 = all_data['output1']
    op1 = all_data['output2']
    op2 = all_data['output3']
    op = np.concatenate((np.expand_dims(op0, 2), np.expand_dims(op1, 2), np.expand_dims(op2, 2)), axis=2)
    ...
    indt2[ind] = np.arange(len(ind), 0, -1)
    st=0.33
    indt2[ind[np.arange(int(len(ind) * st), int(len(ind) * st )+ num)]]= np.arange(0,  -num, -1)

    indt=0.01*indt+indt2
    ind = np.argsort(indt)[::-1]
    inds_train = np.arange(len(ind) - num, len(ind)).astype(int)
    inds_test = np.setdiff1d(ind, inds_train).astype(int)
    return ind,inds_train,inds_test

def third_sel(indtest,wt,num):
    ...
    all_data = sio.loadmat('prompt3vote_rt25_thirdstep_livew.mat')
    op0 = all_data['output1']
    op1 = all_data['output2']
    op2 = all_data['output3']
    op = np.concatenate((np.expand_dims(op0, 2), np.expand_dims(op1, 2), np.expand_dims(op2, 2)), axis=2)
...
def main():
    ...
    ind_bf,inds_train_bf,inds_test_bf=first_sel()
    test_bf=ind_bf[inds_test_bf]

    rt = 0.1
    num=int(X.shape[0]*rt)
    ind,inds_train_new,inds_test_new=second_sel(test_bf,0,num)
    ind_train=np.concatenate((ind_bf[inds_train_bf],test_bf[ind[inds_train_new]]),axis=0)
    ind_test=test_bf[ind[inds_test_new]]

    rt = 0.1
    num = int(X.shape[0] * rt)
    ind_third,inds_train_third,inds_test_third=third_sel(ind_test,0.005,num)
    ind_train = np.concatenate((ind_bf[inds_train_bf], test_bf[ind[inds_train_new]], ind_test[ind_third[inds_train_third]]), axis=0)
    ind_test = ind_test[ind_third[inds_test_third]]
    ...
    for i in range(3):
        ...
        model.load_state_dict(torch.load('livew244_rt25_thirdstep10.005.pt'))
        ###################################################################
        ...
        train_loader = DataLoaderX(train_dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers_train,
                                   pin_memory=True)
        test_loader = DataLoaderX(test_dataset, batch_size=batch_size * 6, shuffle=False, num_workers=num_workers_test,
                                  pin_memory=True)
        ...
        for epoch in range(epochs):
            ...
            if max_plsp < pl + pl2:
                save_nm = 'livew244_rt25_ftencoder' + str(i) + '.pt'
                max_plsp = pl + pl2
