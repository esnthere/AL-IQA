...
class PromptLearner(nn.Module):
    def __init__(self, classnames,clip_model,  ctx_init,class_token_position):
        super().__init__()
...
class CustomCLIP(nn.Module):
    def __init__(self, classnames, clip_model):
        super().__init__()
        self.prompt_learner0 = PromptLearner(classnames, clip_model,"this photo is", 'end')
        self.tokenized_prompts0 = self.prompt_learner0.tokenized_prompts

        self.prompt_learner1 = PromptLearner(classnames, clip_model, "photo",'front')
        self.tokenized_prompts1 = self.prompt_learner1.tokenized_prompts

        self.prompt_learner2 = PromptLearner(classnames, clip_model, "this is a photo",'middle')
...
class CustomCLIP2(nn.Module):
    def __init__(self, classnames, clip_model):
        super().__init__()
        self.prompt_learner0 = PromptLearner(classnames, clip_model,"It's a photo of", 'end')
        self.tokenized_prompts0 = self.prompt_learner0.tokenized_prompts
...
class CustomCLIP3(nn.Module):
    def __init__(self, classnames, clip_model):
        super().__init__()
        self.prompt_learner0 = PromptLearner(classnames, clip_model,"the noise in this photo is", 'end')
        self.tokenized_prompts0 = self.prompt_learner0.tokenized_prompts

        self.prompt_learner1 = PromptLearner(classnames, clip_model, "noise is in this photo",'front')
        self.tokenized_prompts1 = self.prompt_learner1.tokenized_prompts

        self.prompt_learner2 = PromptLearner(classnames, clip_model, "this photo has noise", 'middle')
        self.tokenized_prompts2 = self.prompt_learner2.tokenized_prompts
...
def main():
    ...
    test_loader = DataLoader(Mydataset(X, Y), batch_size=128 , shuffle=False, num_workers=0, pin_memory=True)
    ...
    op1=[]
    op2=[]
    classnames = [['bright', 'dark'], ['sharp', 'blurry'], ['clear', 'unclear'], ['good', 'bad']]
    for classname in classnames:
        print(classname)
        model = nn.DataParallel(CustomCLIP(classname, clip_model)).to(device)

        # pretrained_dict =torch.load( 'koniq244_rt01_voteselect2.pt')
    ...
    classname = ['obvious', 'unobvious']
    model = nn.DataParallel(CustomCLIP3(classname, clip_model)).to(device)
    print(classname)
    op0t, op1t, op2t = test(model, test_loader, -1, device, [])
    op0.append(op0t)
    op1.append(op1t)
    ...
    classname = ['high quality', 'low quality']
    print(classname)
    model = nn.DataParallel(CustomCLIP2(classname, clip_model)).to(device)
    op0t, op1t, op2t = test(model, test_loader, -1, device, [])
    op0.append(op0t)
    op1.append(op1t)
    op2.append(op2t)
    ...
    sio.savemat('prompt3vote_livew.mat',{'output1':op0,'output2':op1,'output3':op2})
